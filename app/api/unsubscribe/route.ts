...
export async function GET(request: Request) {
  try {
    ...
    return new NextResponse(error instanceof Error ? error.message : "Unexpected unsubscribe error.", {
      status: 500,
      headers: {
        "Content-Type": "text/plain; charset=utf-8"
      }
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Unexpected unsubscribe error." },
      { status: 500 }
    );
  }
}
